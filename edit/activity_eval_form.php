...
class mform_activity_eval extends moodleform {
    ...
    public function definition() {
		...
		// Remediation
		if (!$complementary) {
	        $mform->addElement('checkbox', 'remedial', get_string('remedial', 'trainingpath'));
		}
		...
		// Duration
		$error = get_string('eval_duration_error', 'trainingpath');
		$mform->addElement('text', 'duration', get_string('eval_duration','trainingpath'), 'maxlength="3" size="3"');
		$mform->setType('duration', PARAM_INT);
		$mform->setDefault('duration', $sl_config->maxtime);
		...
		$mform->addHelpButton('duration', 'eval_duration', 'trainingpath');
		
		// Chrono
		$mform->addElement('selectyesno', 'displaychrono', get_string('displaychrono', 'scormlite'));
		$mform->setDefault('displaychrono', $sl_config->displaychrono);
        ...
        $mform->setDefault('maxattempt', $sl_config->maxattempt);
        ...
        $mform->setDefault('whatgrade', $sl_config->whatgrade);

		// Passing score
		$mform->addElement('text', 'passingscore', get_string('passingscore','scormlite'), 'maxlength="2" size="2"');
		$mform->setDefault('passingscore', $sl_config->passingscore);
		...
		$mform->addRule('passingscore', null, 'required', null, 'client');
		...
		if ($complementary) {
			...
			$mform->addElement('hidden', 'evaluation', 1);
			$mform->setType('evaluation', PARAM_INT);
		}
