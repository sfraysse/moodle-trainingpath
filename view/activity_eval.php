...
// Page URL
$url = new moodle_url('/mod/trainingpath/view/activity_eval.php', array('cmid'=>$cmid, 'via'=>$via, 'activity_id'=>$activity_id));
$PAGE->set_url($url);
...
if ($item->complementary) {
	$breadcrumb[] = array('label'=>get_string('eval', 'trainingpath'));
	$title = $item->title;
} else {
...
$statsUrl = new moodle_url('/mod/trainingpath/report/eval_statistics.php', array('cmid'=>$cmid, 'activity_id'=>$activity_id, 'via'=>$via));
$commands = array();
...
if (has_capability('mod/trainingpath:addinstance', context_module::instance($cmid))) {
	if (!$learningpath->locked) $commands[] = (object)array('href'=>$editUrl, 'class'=>'secondary', 'title'=>get_string('edit', 'trainingpath'));
	$commands[] = (object)array('href'=>$statsUrl, 'class'=>'secondary', 'title'=>get_string('statistics', 'trainingpath'));
}
