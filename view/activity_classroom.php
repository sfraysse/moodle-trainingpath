...
// Page URL
$url = new moodle_url('/mod/trainingpath/view/activity_classroom.php', array('cmid'=>$cmid, 'via'=>$via, 'activity_id'=>$activity_id));
$PAGE->set_url($url);
...
if ($item->complementary) {
	$breadcrumb[] = array('label'=>get_string('classroom', 'trainingpath'));
	$title = $item->title;
} else {
