...
// Page URL
$url = new moodle_url('/mod/trainingpath/view/batches.php', array('cmid'=>$cmid));
$PAGE->set_url($url);
...
trainingpath_trigger_path_event('page_viewed', $course, $cm, $learningpath, ['page' => 'phases']);


//------------------------------------------- Page setup -------------------------------------------//

trainingpath_view_setup_page($course, 'batches');
...
// Items
$openUrl = (new moodle_url('/mod/trainingpath/view/sequences.php', array('cmid'=>$cmid, 'via'=>'batches')))->out().'&batch_id=';
$editUrl = (new moodle_url('/mod/trainingpath/edit/batch.php', array('cmid'=>$cmid)))->out().'&batch_id=';
echo trainingpath_view_get_items(EATPL_ITEM_TYPE_BATCH, $course, $cm, $learningpath, $topItem->id, $openUrl, $editUrl);

// Edit button
if (!$learningpath->locked) {
    $url = new moodle_url('/mod/trainingpath/edit/batches.php', array('cmid'=>$cm->id));
    $commands = trainingpath_view_get_edit_commands($cmid, get_string('edit_batches', 'trainingpath'), $url);
