...
// Page URL
$url = new moodle_url('/mod/trainingpath/view/activity_richtext.php', array('cmid'=>$cmid, 'via'=>$via, 'activity_id'=>$activity_id));
$PAGE->set_url($url);
...
if ($item->complementary) {
	$breadcrumb[] = array('label'=>get_string('richtext', 'trainingpath'));
	$title = $item->title;
} else {
...
// Content
$item->information = file_rewrite_pluginfile_urls($item->information, 'pluginfile.php', $context_module->id, 'mod_trainingpath', 'richtext', $item->id);
$content = format_text($item->information, 1, array('noclean'=>true, 'context'=>$context_module));
echo trainingpath_get_div($content, 'activity-content');
