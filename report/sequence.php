...
// Page URL
$urlParams = array('cmid'=>$cmid, 'group_id'=>$group_id, 'sequence_id'=>$sequence_id, 'eval_only'=>$evalOnly);
$url = new moodle_url('/mod/trainingpath/report/sequence.php', $urlParams);
$PAGE->set_url($url);
...
if ($format == 'lms') {
    ...
    $breadcrumb[] = array('label'=>$certificate->code, 'url'=>(new moodle_url('/mod/trainingpath/report/certificate.php', array('cmid'=>$cmid, 'group_id'=>$group_id, 'certificate_id'=>$certificate->id, 'eval_only'=>$evalOnly)))->out());
...
if ($format == 'lms') {
    ...
    // Switch
    $switchUrl = (new moodle_url('/mod/trainingpath/report/sequence.php', array('cmid'=>$cmid, 'group_id'=>$group_id, 'sequence_id'=>$sequence_id, 'eval_only'=>!$evalOnly)))->out();
    echo '<p><a href="'.$switchUrl.'">'.get_string('eval_only_sequence_switch_'.$evalOnly, 'trainingpath').'</a></p>';
    ...
    // Print comment
    echo trainingpath_report_comments_get_form('/mod/trainingpath/report/sequence.php', $urlParams, $sequence_id, EATPL_ITEM_TYPE_SEQUENCE, null, $group_id);

    // Exports
    $exports = array();
    $exports[] = (object)array('title'=>get_string('xls_export', 'trainingpath'), 'format'=>'xls', 'url'=>'/mod/trainingpath/report/sequence.php', 'params'=>$urlParams);
